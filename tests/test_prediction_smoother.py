...
def _push(
    smoother: PredictionSmoother, label: str, confidence: float, count: int
) -> list:
    """Push `count` identical predictions; return all results."""
    return [smoother.update(label, confidence) for _ in range(count)]


def _last(smoother: PredictionSmoother, label: str, confidence: float, count: int):
...
class TestMajorityVote:
    def test_majority_label_wins(self):
        s = PredictionSmoother(buffer_size=5, majority_threshold=0.6, cooldown_frames=0)
        # 3 x 'cat', 2 x 'dog' -> cat wins (3/5 = 0.6 >= threshold)
        _push(s, "dog", 0.5, 2)
    ...
    def test_tie_returns_none(self):
        ...
        # 2 x 'a', 2 x 'b' -> neither reaches 60%
        _push(s, "a", 0.8, 2)
...
class TestClear:
    ...
    def test_clear_resets_cooldown(self):
        s = PredictionSmoother(
            buffer_size=5, majority_threshold=0.6, cooldown_frames=10
        )
